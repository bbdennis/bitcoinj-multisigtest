...
import multisigtest.utils.TextFieldValidator;
...
import static multisigtest.utils.GuiUtils.*;

public class Main extends Application {
    public static String APP_NAME = "MultisigTestWallet";
    public static File WALLET_LOCATION = new File(".");
    public static File SHAREFOLDER_LOCATION = new File(".");

    public static ECKey myKey;
...
//        bitcoin = new WalletAppKit(params, new File("."), APP_NAME + "-" + params.getPaymentProtocolId()) {
        bitcoin = new WalletAppKit(params, WALLET_LOCATION, APP_NAME + "-" + params.getPaymentProtocolId()) {
            @Override
    ...
    public static void main(String[] args) {
        String walletFullPath = System.getProperty("WalletFullPath");
        String walletName = System.getProperty("WalletName");
        String shareFolderFullPath = System.getProperty("ShareFolderFullPath");

        if (walletName!= null && walletName.trim().length()>0) {
            APP_NAME=walletName;
        }

        if (walletFullPath != null && walletFullPath.trim().length() > 0) {
            Path walletPath = Paths.get(walletFullPath);
            if (Files.exists(walletPath)) {
                WALLET_LOCATION = new File(walletFullPath);
            }
        }

        if (shareFolderFullPath != null && shareFolderFullPath.trim().length() > 0) {
            Path sfPath = Paths.get(shareFolderFullPath);
            if (Files.exists(sfPath)) {
                SHAREFOLDER_LOCATION = new File(shareFolderFullPath);
            }
        }
