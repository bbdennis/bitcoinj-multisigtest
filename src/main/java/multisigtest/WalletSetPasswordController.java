...
import multisigtest.utils.KeyDerivationTasks;
...
public class WalletSetPasswordController {
    ...
    public void setPasswordClicked(ActionEvent event) {
        ...
        if (password.length() < 4) {
            GuiUtils.informationalAlert("Password too short", "You need to pick a password at least five characters or longer.");
            return;
        }

        GuiUtils.fadeIn(progressMeter);
        GuiUtils.fadeOut(widgetGrid);
        GuiUtils.fadeOut(explanationLabel);
        GuiUtils.fadeOut(closeButton);
        ...
        KeyDerivationTasks tasks = new KeyDerivationTasks(scrypt, password, estimatedKeyDerivationTime) {
            ...
            protected void onFinish(KeyParameter aesKey, int timeTakenMsec) {
                ...
                GuiUtils.informationalAlert("Wallet encrypted",
